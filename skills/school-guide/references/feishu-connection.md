...
- 没有可用配置：解释“首次需要建立飞书应用连接；可以由管理员安全配置已有应用，或创建你自己的应用”。先取得用户对创建个人应用的明确同意，再执行 `lark-cli config init --new --name fde-school --brand feishu`。这不是加入已经部署好的统一应用；未同意创建或组织只允许现有应用时，等待管理员安全配置。

配置命令可能等待浏览器操作。后台运行并读取真实验证链接后发给学员，继续恢复同一进程，不重复启动。若 CLI 提示当前 Agent 已有应用、应使用 `config bind`，先核对帮助并复用；不要默认加 `--force-init` 创建平行配置。不要输出配置中的密钥。组织禁止创建应用或要求审核时，说明当前报错与所需管理员动作，不尝试绕过；仍可让学员通过学校首页阅读。
...
- 缺目录能力且返回要求 `space:document:retrieve`：可单独发起 `lark-cli --profile fde-school auth login --scope space:document:retrieve --no-wait --json`（若复用其他 profile，替换为该名称）并恢复同一流程；若应用未获批该能力，个人重复登录不能代替审批。
