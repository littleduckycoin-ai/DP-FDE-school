...
学校目录索引现为3.1：每例有稳定的`case_id`，基础JSON和Markdown位于`cases/<case_id>/base/`，另有`reflections_dir`、`canon_dir`与`online_urls`供Agent在线路由。案例内容结构仍为2.1，原访谈不变；学员反馈直接追加为`reflections/<learner-id>-YYYY-MM-DD.md`知识文件，沿用1.0 Markdown+JSON格式，见[记录结构](../docs/reflection-format.md)，不混入案例事实字段。
