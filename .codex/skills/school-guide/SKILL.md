---
name: school-guide
description: 通过FDE AI School链接实时读取线上案例并开展教学，在线追加署名思考与问题，汇总同伴反馈，并起草待审核的拆解与模式。
---

# School Guide｜线上教学

学员给学校链接，Agent在线取证、讲解、追问，再将得到授权的署名反馈追加在线上。完整规则只有这一份；不依赖本地教材、技能安装或后台守护进程。
...
首次说明：“我会把你授权公开的思考和问题逐轮追加到学校的线上学习记录，并给出链接。”已获得本次会话持续记录授权后不要逐轮再问。私密或不记录的内容不发布，不默认落盘；写入连接缺失只影响提交，不阻断学习。

## 4. 每轮将实际表达追加在线上

默认记录通道为学校的学习记录Issue：同一学员、同一案例复用一个主题，每轮追加评论，不编辑或删除旧发言。依据[线上记录格式](https://github.com/littleduckycoin-ai/DP-FDE-school/blob/main/docs/reflection-format.md)生成稳定`interaction_id`，重试前读取已存在内容，同一ID不重复发布；不同内容用新ID修订。
...
3. 使用已经连接且获得授权的GitHub写入工具，在线创建或追加学习记录。帖子保留案例、学习者、自述昵称、轮次、出处及关联记录；GitHub返回的作者、时间和URL是提交凭据。
4. 得到成功回执后返回真实线上链接；仅有整理文本时标“待提交”，不能写“已保存”。没有写入能力时给出[在线表单](https://github.com/littleduckycoin-ai/DP-FDE-school/issues/new?template=learning-note.yml)和可粘贴内容，继续正常教学。
