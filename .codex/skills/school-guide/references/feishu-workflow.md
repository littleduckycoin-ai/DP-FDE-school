...
本学校只使用飞书。脚本内置飞书 Agent 目录；每次开课直接读取线上目录与规则，不依赖其他资料中台。

学校提供 `scripts/feishu_school.mjs` 处理重复的目录读取和记录操作。脚本属于本 skill；学员无需再安装另一套飞书 skill。首次使用先读取[连接与首次授权](feishu-connection.md)，已有可用连接则直接继续。

## 在线入口和全文读取

通过脚本的 bootstrap 能力读取学校已配置的入口，获取规则、案例索引与本次连接状态。规则来自飞书已配置的学校规则文档，参考文件与脚本是执行方法，案例和学习记录仍在线读取。

学校脚本接口如下，由 Agent 执行；manifest 默认使用内置学校入口；维护者明确指定其他飞书入口时才覆盖，不让学员查找内部标识。首次运行核对脚本 `--help`，以当前发布实现为准。

| 任务 | 脚本子命令 |
|---|---|
| 当前入口与规则 | `bootstrap`；默认读取内置飞书入口 |
| 缺项诊断 | `doctor` |
...
每个需要资料的回合检查当前线上规则和资料版本；记录获取时间、文档 ID、可获得的 revision/修改时间及内容摘要。飞书多个文档不天然共享同一版本；只有工具已验证一致性时才声称同一快照，否则说明本次实际读取窗口。失败时说明读取范围，不以本地教材或旧副本冒充当前飞书材料。
...
如果因 OAuth、文档权限、网络或接口限制不能写，Agent 先处理能够代办的连接步骤，剩余部分请用户完成相应授权。仍失败时给完整待提交正文、稳定 interaction ID 和目标文档/父节点链接，保持“待提交”；manifest 本身无权读取时提供已知入口与 `case_id`、标记“目标待解析”。不转存其他平台或飞书评论。不要让用户为每轮学习手工运行命令。
...
优先使用飞书 `case` 返回的 `source_page_links: [{pdf_page, url}]`，按对应物理页选择已验收的链接。其目标可以是从原 PDF 保真提取的单页 PDF；打开即可看到那一页，无需添加未经验证的页码参数。
