...
| 已连接GitHub且具备相应写权限 | 在一次明确授权后，创建或更新案例reflection文件并返回文件或PR链接 |
| 能读但不能写 | 正常学习；Agent给出准确路径与完整Markdown，学员从GitHub网页提交文件或PR |
| 只能检索搜索摘要或不能联网 | Agent应说明读取受限，引导使用可直接访问学校的工具；不能声称已读到全文 |

读取公开资料不要求学员先登录GitHub；发布reflection需要对应GitHub账号或已授权连接。聊天链接不会自动授予写权限。这里的“初始化”是Agent在线读教学说明、了解学习目标，并按需确认署名与记录授权。
