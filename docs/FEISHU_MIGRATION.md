...
迁移工具默认在指定目标下面新建 `FDE AI School` 子目录。如果已经为学校新建了一个专用空文件夹，可以在本地、未提交到 Git 的 target JSON 中明确设置：

```json
{
  "parent": {"type": "folder", "token": "已确认的新空文件夹token"},
  "use_parent_as_school": true
}
```

工具会完整读取目录，确认为空后先记入迁移 journal，再把资料直接迁入该文件夹。非空且没有对应 journal 的目录会被拒绝；不会根据同名文件猜测归属。续传必须继续使用同一 plan、target 和 state，此时按 journal 复用已采用的学校目录。此选项只支持文件夹；Wiki 仍使用默认的新建子节点方式。不要在已开始的迁移中更改此选项，也不要删除 journal 后尝试接管已迁入资料的目录。

迁移 manifest 会记录真实的 `patterns_parent`、`meetings_parent` 和 `curriculum_parent`，供学习助手定位模式提案、会议资料和课程资料。目录存在不代表已授予团队访问权限；完成实际验收后才切换学校入口。

如果本次迁移要求所有资料仅本人可见，在同一份本地 target JSON 中增加 `"private_owner_open_id": "本人经 OAuth 远端核验的 open_id"`。工具先验证当前登录者与该标识一致，再逐个检查目标文件夹、子文件夹、文档和附件；续传也会重新检查已完成资源。实际权限检查会写入本地 journal，不会仅根据父文件夹推断子文件权限。

此模式要求协作者列表只有该本人且具有 `full_access`、链接分享为 `closed`。Docx 和文件必要时使用官方权限接口关闭链接与外部访问，再读回确认；文件夹只检查，因为当前官方修改接口未声明支持 `folder`。发现其他协作者、读取不完整、文件夹未关闭链接或不支持的资源类型时会停止，不自动删除协作者或猜测接口。Wiki 暂不支持此模式。此权限状态是检查时的结果，不能阻止所有者随后手动改变分享设置。

迁移只操作明确指定的学校目录及其已核实子目录，不搜索全云盘、企业知识库或其他数据区域。journal 为资源保存固定父目录；每次读取正文或修改权限前，都按实际目录中的资源 ID 核验归属，不能靠修改 journal 或文档 URL 把其他区域的内容带入迁移。缺少父目录证据的旧 journal 需要维护者明确核对后处理，工具不会自动接管。
